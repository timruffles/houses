var Classifier, assert, cameron, classifier, classify, exampleTweet, fs;
...
cameron.forEach(function(_arg) {
  ...
  category = isDavidCameron ? "interesting" : "boring";
  /* console.log "training #{line.substr(0,20)} as #{category}"*/;
  return classifier.train(1, {
    text: line,
    user: {},
    entities: {}
  }, category);
...
classifier.on("classified", function(tweet, searchId, category) {
  return /* console.log("#{category}: #{tweet.text}")*/;
});
