...
fs = require "fs"
Classifier = require("../app/js/classifier").Classifier

classifier = new Classifier {query: ->}

# test classification string
classifier.classificationString exampleTweet = {
  "coordinates": null,
...
cameron.forEach ([isDavidCameron,line]) ->
  category = if isDavidCameron then "interesting" else "boring"
  /* console.log "training #{line.substr(0,20)} as #{category}"*/
  classifier.train 1, {text:line,user: {},entities: {}}, category

classify = fs.readFileSync "to_classify.txt", "utf8"
classify = classify.split("\n")
classifier.on "classified", (tweet, searchId, category) ->
  /* console.log("#{category}: #{tweet.text}")*/
classify.forEach (line) ->
  category = classifier.classify {text:line,user:{},entities: {}},1
